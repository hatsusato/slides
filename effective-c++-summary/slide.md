...
- 組み込み型に対して初期化式を指定しないと、Cと同様にオブジェクトの値は不定になります。
  必ず初期化式を与えるようにしましょう。
  - Cとは違い、C++では任意の位置でオブジェクトを初期化できることに気をつけましょう。
...
## 16項 対応する`new`と`delete`は同じ形のものを使おう
...
## 17項 `new`で生成したオブジェクトをスマートポインタに渡すのは、独立したステートメントで行うようにしよう
...
## 23項 メンバ関数より、メンバでも`friend`でもない関数を使おう
